...
from src.data_utils import add_gaussian_noise, load_grayscale_image
...
def _load_selected_image_paths(data_dir: str | Path, selected_images_file: str | Path) -> list[Path]:
    ...
    names = [
        line.strip()
        for line in Path(selected_images_file).read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    if not names:
        raise ValueError(f"No image names found in {selected_images_file}.")
